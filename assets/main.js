...
const titlebar = "<div class=\"title-bar\"><p id=\"title-bar-title\"></p><a onmousedown=\"return false\" href=\"#\" onclick=\"closeWindow()\"><img src=\"./assets/img/close.png\"></a></div>"
    ...
    function showWebsite(url) {
        ...
        } else if (url.indexOf('changelog') == 0) {
          webview.loadURL("file://"+__dirname+"/changelog.html");
          ...
          success: function(data) {
            ...
            if (matches == null) {
              $.getJSON(__dirname+"/assets/colors.json", function(data) {
                for (i in data) {
                  ...
                  if ("http://"+data[i].url == link) {
                    url = "http://besticon-demo.herokuapp.com/lettericons/"+link.charAt(7).toUpperCase()+"-64-"+ data[i].color.substr(1).slice(0) +".png";
                    ...
                    url = "http://besticon-demo.herokuapp.com/lettericons/"+link.charAt(8).toUpperCase()+"-64-"+ data[i].color.substr(1).slice(0) +".png";
                    console.log("[no proto] color for " + data[i].url + " : " + data[i].color);
                    console.log(url+"\n\n");
                    $(document).find('#' + key).css('background-image', "url(" + url + ")");
                    ...
                    url = "http://besticon-demo.herokuapp.com/lettericons/"+link.charAt(0).toUpperCase()+"-64-"+ data[i].color.substr(1).slice(0) +".png";
                    console.log("[no proto] color for " + data[i].url + " : " + data[i].color);
                    console.log(url+"\n\n");
                    $(document).find('#' + key).css('background-image', "url(" + url + ")");
              ...
              getColors('https://www.google.com/s2/favicons?domain=' + link.toLowerCase()).then(colors => {
                ...
                url = "http://besticon-demo.herokuapp.com/lettericons/"+matches[1].charAt(0).toUpperCase()+"-64-"+ colors[4].hex().substr(1).slice(0) +".png";
                  ...
                  $.getJSON(__dirname+"/assets/colors.json", function(data) {
                     for (i in data) {
                      ...
                      if ("http://"+data[i].url == link) {
                          ...
                          url = "http://besticon-demo.herokuapp.com/lettericons/"+matches[1].charAt(0).toUpperCase()+"-64-"+ data[i].color.substr(1).slice(0) +".png";
                          console.log("[no proto] color for " + data[i].url + " : " + data[i].color);
                          console.log(url+"\n\n");
                          $(document).find('#' + key).css('background-image', "url(" + url + ")");
