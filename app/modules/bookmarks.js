...
const bkms = {
  ...
  getIcon (url, callback) {
    let icon = 'lol'

    axios.get(`http://favicongrabber.com/api/grab/${require('url').parse(url).hostname}`)
      .then(function (response) {
      // handle success
        let json = response.data
        if ('icons' in json && json.icons.length > 0) {
          icon = json.icons[0]

          console.log(`[INFO] > Icon for ${url} : ${icon.src}`)
        }
  ...
  loadBookmarks () {
    return new Promise((resolve) => {
      ...
      sync.syncBookmarks().then((bkm) => {
        ...
        for (let order in sortedindexes) {
          let i = sortedindexes[order]
